...
void Graph::lerArquivo(string caminho)
{
    ...
    if(arquivo.is_open()){
        ...
        if (tipo_arquivo == "net" || tipo_arquivo == "paj"){
            ...
            this->qtd_ligacoes=0;
            unsigned int v1, v2;
            this->menorPesoDeAresta = 0;
            this->maiorPesoDeAresta = 0;

            do{
                v1 = atoi(campos[0].c_str())-comecaCom;
                v2 = atoi(campos[1].c_str())-comecaCom;

                int indicetual = listaArestas.size();
                /////cout << "Aresta criada: " << "v1: " << v1 <<" v2: "<< v2 << endl;
                this->listaArestas.push_back(Aresta(v1,v2, listaArestas.size()));

                //v1.addAresta(Aresta(v1,v2,id listaArestas.size()));
                //v2.addAresta(Aresta(v1,v2, listaArestas.size()));

                if (this->ponderado){
                    this->somatorio_pesos += atof(campos[2].c_str());
                    this->matrizAdj[v1][v2] = atof(campos[2].c_str());

                    if (this->menorPesoDeAresta == 0){
                        this->menorPesoDeAresta = atof(campos[2].c_str());
                    }

                    if (this->matrizAdj[v1][v2] > this->getMaiorPesoDasArestas()){
                        this->maiorPesoDeAresta = this->matrizAdj[v1][v2];
                    }

                    if (this->matrizAdj[v1][v2] < this->getMenorPesoDasArestas()){
                        this->menorPesoDeAresta = this->matrizAdj[v1][v2];
                    }

                    //cout << "Vertice 1: "<< v1 <<  "  Vertice 2: " << v2 << "  Peso: " << matrizAdj[v1][v2] << endl;
                ...
                if (this->dirigido == false){
                    if (this->ponderado){
                        ...
                        if (this->matrizAdj[v2][v1] > this->getMaiorPesoDasArestas()){
                            this->maiorPesoDeAresta = this->matrizAdj[v1][v2];
                        }

                        if (this->matrizAdj[v2][v1] < this->getMenorPesoDasArestas()){
                            this->menorPesoDeAresta = this->matrizAdj[v1][v2];
                        }
                    ...
                    //cout << "vertice: " << v1 << " aresta id:  " <<  indicetual << endl;

                    this->listaVertices[v1].addArestaPropria(indicetual);
                    //this->listaVertices[v2].addArestaPropria(indicetual);

                    //this->listaVertices[v2].addArestaPropria(this->listaArestas[listaArestas.size()]);
                }else{
                    if (this->ponderado){
